...
import { getSupplierPrompt } from './prompts/supplier.v1';
...
export const executeSupplierAgent = async (context: RouterContext, entities: string[]) => {
  // simplistic implementation: just fetch all entities for context
  const data = await queryByPrefix(context.tenantId, 'ENTITY#', { limit: 20 });
  const contextStr = JSON.stringify(data.items);
  
  const systemPrompt = getSupplierPrompt(contextStr);
